...
class BaseDataset(Dataset):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    ...
                    #im = imread(f, flags=self.cv2_flag)  # BGR     4通道屏蔽
            else:  # read image
                im_rgb = cv2.imread(f)  # BGR
                im_ir = cv2.imread(f.replace('rgb', 'ir'), cv2.IMREAD_GRAYSCALE)  # ir
                
                if im_rgb is None or im_ir is None:
                    raise FileNotFoundError(f"Image Not Found {f}")

                h_vis, w_vis = im_rgb.shape[:2]  # orig hw
                h_inf, w_inf = im_ir.shape[:2]  # orig hw
            ...
            if im.shape[2] != 4:
                raise ValueError(f"Expected 4 channels, but got {im.shape[2]} channels")
    ...
    def check_cache_ram(self, safety_margin=0.5):
        ...
        for _ in range(n):
            ...
            b += (im.nbytes * 4/3) * ratio**2   #4通道适应
        mem_required = b * self.ni / n * (1 + safety_margin)  # GB required to cache dataset into RAM
        mem = __import__("psutil").virtual_memory()
        if mem_required > mem.available:
            self.cache = None
            LOGGER.warning(
                f"4-channel memory caching failed due to not enough rom. "
