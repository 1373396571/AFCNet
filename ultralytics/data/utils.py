...
def verify_image_label(args):
    ...
    try:
        ...
        #验证红外
        im_ir = Image.open(im_file.replace('rgb', 'ir'))
        im_ir.verify()  # PIL verify
        shape_ir = exif_size(im_ir)  # image size
        shape_ir = (shape_ir[1], shape_ir[0])  # hw
        assert (shape_ir[0] > 9) & (shape_ir[1] > 9), f"image_ir size {shape_ir} <10 pixels"
        assert im_ir.format.lower() in IMG_FORMATS, f"invalid image_ir format {im_ir.format}. {FORMATS_HELP_MSG}"
        if im_ir.format.lower() in {"jpg", "jpeg"}:
            with open(im_file.replace('rgb', 'ir'), "rb") as f_ir:
                f_ir.seek(-2, 2)
                if f_ir.read() != b"\xff\xd9":  # corrupt JPEG
                    raise ValueError(f"Corrupt JPEG file detected: {im_file.replace('rgb', 'ir')}")
...
def check_det_dataset(dataset, autodownload=True):
    ...
    ######################################################
    data["channels"] = data.get("channels", 4)  # get image channels, default to 4
    #####################################$$$$$$$$$$$$$$$$$$$$$$$
    # Resolve paths
